...
const EmployeeProfileMob = () => {
  ...
  const [openCardIndex, setOpenCardIndex] = useState(null); // State to manage which card is open (null means all closed)

  const toggleCard = (index) => {
    setOpenCardIndex(openCardIndex === index ? null : index); // If the same card is clicked, it will close; else open the new card
  };
  useEffect(() => {
                ...
                <div className="col-6 col-md-4 mb-4" key={index}>
                  <div className="card">
                    <div
                      ...
                      onClick={() => toggleCard(index)} // Toggle card open/close
                    >
                    ...
                    {openCardIndex === index && ( // Render the body only if this card is open
                      <div className="card-body">
                        {brand.products.map((product, idx) => (
                          <div key={idx} className="accr_inner_text">
                            <ul>
                              <li>
                                <strong>{product.product_name}</strong>
                                <strong> - </strong>
